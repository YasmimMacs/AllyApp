...
export default function ConfirmCode({ route, navigation }: Props) {
  ...
  async function handleConfirm() {
    try {
      ...
      navigation.replace('Home'); // ou Dashboard
    } catch (e: any) {
      alert(e.message);
    }
  }
...
const styles = StyleSheet.create({
  wrapper: { flex: 1, backgroundColor: COLORS.bgLight, justifyContent: 'center', padding: 24 },
  title:   { fontSize: scale(20), fontWeight: '600', color: COLORS.primary, textAlign: 'center', marginBottom: 24 },
  input:   {
    borderWidth: 1, borderColor: COLORS.textGrey, borderRadius: 8,
    padding: 14, fontSize: scale(16), backgroundColor: '#fff', marginBottom: 24,
  },
